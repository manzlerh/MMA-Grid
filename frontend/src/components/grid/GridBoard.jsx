...
const labelClass =
  'aspect-square bg-ufc-gold/10 border border-ufc-gold/30 flex items-center justify-center text-ufc-gold font-semibold text-xs md:text-sm text-center p-1 truncate'
...
export default function GridBoard({ puzzle = {}, board = [], onCellClick, lockedCells = new Set(), shakingCell = null }) {
  ...
  return (
    <div className="grid grid-cols-4 gap-px w-full max-w-md mx-auto aspect-square max-h-[min(90vw,28rem)]">
      ...
      {/* Top row: column labels — truncate on mobile, tooltip for full text */}
      {[0, 1, 2].map((c) => (
        <div
          key={`col-${c}`}
          ...
          {[0, 1, 2].map((c) => {
            ...
            const cellContent = (
              <button
                ...
                className={`w-full aspect-square flex items-center justify-center text-center p-1 transition-colors text-xs md:text-sm
                  ${isEmpty
